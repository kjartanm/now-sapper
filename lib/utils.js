...
exports.getLauncherFiles = function getLauncherFiles (entrypointDir) {
  const launcherPath = path.join(__dirname, 'launcher.js')
  const compiled = fs.readFileSync(launcherPath, { encoding: 'utf-8' }).replace(new RegExp('{entrypointDir}', 'g'), '');//entrypointDir);//mountpoint)
  return {
